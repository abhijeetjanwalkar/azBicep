...
resource nwinterface1 'Microsoft.Network/networkInterfaces@2020-06-01' = [for i in range(0,2): {
  name: 'mynwint${i}'
  //name: 'storageName${i}'
  location: resourceGroup().location
  properties:{
...
resource VM1 'Microsoft.Compute/virtualMachines@2020-06-01' = [for i in range(0,2): {
  name: 'myvm${i}'
  location: location
  properties: {
   hardwareProfile: {
